...
if ( $dept_no == "ALL" )
	$sql ="select id, department.name, employee.name, age from employee, department where employee.dept_no = department.dept_no"; // ※ 全部門の従業員を検索
else
	$sql ="select id, department.name, employee.name, age from employee, department where employee.dept_no = department.dept_no and employee.dept_no = '$dept_no'"; // ※ 指定された部門番号（$dept_no）の従業員を検索
